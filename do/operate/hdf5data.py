...
def get_variant_group_name(variant):
    ...
    var_name = "{}-{}-{}-to-{}-{}".format(mol_name,
                                          variant.residue_id,
                                          variant.wildtype_amino_acid.name,
                                          variant.variant_amino_acid.name,
                                          str(hash(variant)).replace('-', 'm'))
...
def store_variant(variant_group, variant):
    ...
    variant_group.attrs['wildtype_amino_acid_name'] = variant.wildtype_amino_acid.name
    variant_group.attrs['variant_amino_acid_name'] = variant.variant_amino_acid.name

    if variant.variant_class is not None:
        variant_group.attrs['variant_class'] = int(variant.variant_class)
...
def load_variant(variant_group):
    ...
    wildtype_amino_acid = amino_acids_by_name[variant_group.attrs['wildtype_amino_acid_name']]
    variant_amino_acid = amino_acids_by_name[variant_group.attrs['variant_amino_acid_name']]

    if 'variant_class' in variant_group.attrs:
        class_ = VariantClass(variant_group.attrs['variant_class'])
    else:
        class_ = None

    variant = PdbVariantSelection(pdb_path, chain_id, residue_number, insertion_code, wildtype_amino_acid, variant_amino_acid, pssm_paths_by_chain, class_)
